...
//
//     final datasLists = datasListsFromJson(jsonString);

import 'dart:convert';

List<DatasLists> parsedDataList(String responseBody) {
  final parsed = jsonDecode(responseBody).cast<Map<String, dynamic>>();

  return parsed.map<DatasLists>((json) => DatasLists.fromJson(json)).toList();
}
...
class DatasLists {
  ...
  factory DatasLists.fromJson(Map<String, dynamic> json) => DatasLists(
        id: json["id"],
        nodeId: json["node_id"],
        name: json["name"],
...
final defaultBranchValues =
    EnumValues({"master": DefaultBranch.MASTER, "trunk": DefaultBranch.TRUNK});
...
final loginValues = EnumValues({"JakeWharton": Login.JAKE_WHARTON});

enum NodeId { MDQ6_VX_NLCJ_Y2_N_TC3 }

final nodeIdValues =
    EnumValues({"MDQ6VXNlcjY2NTc3": NodeId.MDQ6_VX_NLCJ_Y2_N_TC3});
